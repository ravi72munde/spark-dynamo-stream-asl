...
import org.apache.spark.streaming.kinesis.dynamostream.KinesisInitialPositions.AtTimestamp
  ...
  override def onStart() {
    ...
    val recordProcessorFactory = new v2.IRecordProcessorFactory {
      override def createProcessor: v2.IRecordProcessor =
        new V1ToV2RecordProcessorAdapter(new KinesisRecordProcessor(receiver, workerId))
    }

//    import com.amazonaws.auth.profile.ProfileCredentialsProvider


    val streamsCredentials: AWSCredentialsProvider = kinesisProvider

    val cloudWatchClient = AmazonCloudWatchClientBuilder.standard
      .withClientConfiguration(kinesisClientLibConfiguration.getCloudWatchClientConfiguration).withRegion(kinesisClientLibConfiguration.getRegionName)
      .build //AmazonCloudWatchClientBuilder.standard.withRegion(kinesisClientLibConfiguration.getRegionName).build

    val dynamoDBClient: AmazonDynamoDB = AmazonDynamoDBClientBuilder.standard()
      .withClientConfiguration(kinesisClientLibConfiguration.getDynamoDBClientConfiguration).withRegion(kinesisClientLibConfiguration.getRegionName)
      .build //AmazonDynamoDBClientBuilder.standard.withRegion(kinesisClientLibConfiguration.getRegionName).build

    val adapterClient = new AmazonDynamoDBStreamsAdapterClient(streamsCredentials,kinesisClientLibConfiguration.getKinesisClientConfiguration)// new ClientConfiguration());

    val streamArn: String = dynamoDBClient.describeTable(new DescribeTableRequest()
      .withTableName(kinesisClientLibConfiguration.getStreamName))
      .getTable
      .getLatestStreamArn

    val workerConfig = new KinesisClientLibConfiguration(kinesisClientLibConfiguration.getApplicationName, streamArn, streamsCredentials, workerId)
